...
public class EComAdminFacadeServiceRSImpl implements EComAdminFacadeServiceRS {
    ...
    public ACHTxDTO doACHTransfer(@PathParam("siteId") Long siteId,
        ...
        ACHTxDTO aCHDTO = null;
        try {
            aCHDTO = this.aCHTXService.getACHDetailsAndTransferXMLRequest(siteId, paymentType, createdBy, machineIp);
        } catch(RuntimeException runtimeException) {
	...
	@WebMethod
	@GET @Path("doVoidCheck/{checkNumber}/{comments}/{modifiedBy}")
	@Produces({MediaType.APPLICATION_JSON})
	public boolean doVoidCheck(@PathParam("checkNumber") Long checkNumber,
		@PathParam("comments") String comments, @PathParam("modifiedBy") String  modifiedBy) {
		return this.ecomService.doVoidCheck(checkNumber, comments, modifiedBy);
	}
