...
public class EComDAOImpl extends AbstractBaseDAOImpl implements EComDAO {
    ...
    public List<Site> getSites() {
        ...
        if (resultList.size() > 0) {
            ...
            while(resultListIterator.hasNext()) {
                ...
                site.setCronTriggerForACHTransfer(this.getString(row[13]));
                site.setCronScheduleForAchTransferEnabled(this.getBoolean(row[14]));
                site.setNode(node);
                sites.add(site);
            }
        }
   ...
   public List<Merchant> getMerchantDetailsBySite(Long siteId) {
    ...
    public List<CheckHistory> getCheckHistories(Long siteId, String fromDate, String toDate, String checkNum, Double checkAmt) {
        ...
        if(resultList.size() > 0) {
            ...
            while(resultListIterator.hasNext()) {
                ...
                checkHistory.setAchTxRefNumber(this.getString(row[16]));
                checkHistory.setAchStatus(this.getString(row[17]));
                checkHistory.setAchResponseSeverity(this.getString(row[18]));
                checkHistory.setAchResponseStatusDesc(this.getString(row[19]));
                checkHistory.setAchResponseErrorDesc(this.getString(row[20]));
                checkHistory.setAchResponseStatusDate(this.getDate(row[21]));
    ...
    public boolean doVoidCheck(Long checkNumber, String comments, String modifiedBy) {
        String yesNo = null;
        Session session = currentSession();
        ...
        query.setParameter("modifiedBy", StringUtils.substring(modifiedBy, 0, 48));
